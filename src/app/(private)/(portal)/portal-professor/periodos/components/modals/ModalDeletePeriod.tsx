...
interface ModalDeletePeriodProps {
  isOpen: boolean
  periodName: string
  isDeleting: boolean
  error: string | null
...
export function ModalDeletePeriod({ isOpen, periodName, isDeleting, error, onClose, onConfirm }: ModalDeletePeriodProps) {
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/5">
        ...
        <div className="flex items-start gap-4 px-6 py-5">
          ...
          <div className="flex-1">
            <h2 className="text-base font-semibold text-slate-900">Excluir período</h2>
            <p className="mt-1 text-sm text-slate-500">
              Tem certeza que deseja excluir o período{' '}
              <span className="font-medium text-slate-700">&quot;{periodName}&quot;</span>?
              Todas as etapas vinculadas também serão removidas. Esta ação não pode ser desfeita.
            </p>
          </div>
          <button
            type="button"
