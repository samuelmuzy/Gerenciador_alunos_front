...
export function PeriodForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mt-6" noValidate>
      {!hasPeriods && (
        <p className="mb-4 text-slate-600">No periods registered yet. Create the first academic period below.</p>
      )}

      <div className="space-y-4 rounded-lg border border-slate-200 bg-slate-50/50 p-4">
        <p className="text-sm font-medium text-slate-700">Academic period details</p>

        <FormField label="Period name" htmlFor="period-name" error={errors.nome?.message}>
          <input
            id="period-name"
            type="text"
            {...register('nome')}
        ...
        <FormField label="Description" htmlFor="period-description" error={errors.descricao?.message}>
          <input
            id="period-description"
            type="text"
            {...register('descricao')}
            placeholder="e.g. First semester of 2025"
            className="input"
          />
        </FormField>

        <div className="grid grid-cols-2 gap-4">
          <FormField label="Start date" htmlFor="start-date" error={errors.data_inicio?.message}>
            <input id="start-date" type="date" {...register('data_inicio')} className="input" />
          </FormField>
          <FormField label="End date" htmlFor="end-date" error={errors.data_fim?.message}>
            <input id="end-date" type="date" {...register('data_fim')} className="input" />
          </FormField>
        </div>

        <FormField label="Number of steps" htmlFor="step-count" error={errors.quantidade_etapas?.message}>
          <input
            id="step-count"
            type="number"
            min={1}
        ...
        {!hasMultipleSteps && (
          <FormField label="Maximum grade" htmlFor="max-grade" error={errors.nota_maxima?.message}>
            <input
              id="max-grade"
              type="number"
              min={1}
        ...
        <FormField label="Passing grade" htmlFor="passing-grade" error={errors.nota_corte?.message}>
          <input
            id="passing-grade"
            type="number"
            min={0}
